...
"Yes, I'm a Presbyterian. My conscience is clear. I am reasonably sure of not having cheated the narative when I gave him the Word of God in exchange for his devilish book."
...
We talked about India, about Orkney, and about the Norwegian jails who once ruled it. It was night when the man left. I have not seen him again, nor do I know his name. 
...
longer recall, elevated to the ninth power. 
...
function getStyledText() {
  ...
  const selected = paragraphs.slice(0, paragraphLength);
  ...
  return selected.map(paragraph => {
    ...
    let result = "";
    let groupSize = Math.floor(Math.random() * 4) + 3; // 3–6 words
    let randomClass = styleClasses[Math.floor(Math.random() * styleClasses.length)];

    for (let i = 0; i < words.length; i++) {

      // Change style every few words result??
      if (i % groupSize === 0) {
        groupSize = Math.floor(Math.random() * 4) + 3;
        randomClass = styleClasses[Math.floor(Math.random() * styleClasses.length)];
      }


      //LETTER SHAKE ANIMATION RESULT????
    const letters = words[i].split("");
    let letterSpans = "";

    for (let l = 0; l < letters.length; l++) {
    const delay = (Math.random() * 0.2).toFixed(2);
    letterSpans += `<span class="shake-letter" style="animation-delay:${delay}s">${letters[l]}</span>`;
}

result += `<span class="${randomClass}">${letterSpans}</span> `;
...
function generateParagraph() {
  document.getElementById("output-1").innerHTML = getStyledText();
}

generateParagraph();

["output-1"].forEach(id => {
document.addEventListener("click", generateParagraph);;
});




//PIXEL GENERATING THING!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
/*
function generatePixels(canvas, count = 500) {
...
const b = 0;
ctx.fillStyle = `#eacaa2`;

    ctx.fillRect(x, y, 4, 4); 
  }
}

// Apply pixels to all canvases
function generateAllPixels() {
  document.querySelectorAll('.pixel-canvas').forEach(canvas => {
    generatePixels(canvas, 1000); // change 1000 for more/fewer pixels
  });
}

// Generate pixels on page load
generateAllPixels();

// Regenerate pixels when you click on a page
document.querySelectorAll('.page-1, .page-2').forEach(page => {
  page.addEventListener('click', generateAllPixels);
});

*/


/* OLD RESHUFFLE PARAGRAPH
...
return paragraphs.slice(0, paragraphLength).join("\n\n");
}

function generateParagraph() {
  document.getElementById("output-1").textContent = getShuffledText();
  document.getElementById("output-2").textContent = getShuffledText();
}

generateParagraph();


["output-1", "output-2"].forEach(id => {
